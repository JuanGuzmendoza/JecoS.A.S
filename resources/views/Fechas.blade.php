...
@section('content')
    ...
    <div id="mydatatable-container">
          ...
          <div class="g_botones">
                <!-- Botón que abre el formulario flotante -->
                  <button type="button" class="btn btn-primary rounded" data-toggle="modal" data-target="#exampleModal">Agregar fila</button>
                ...
                <table id="myTable" class="table table-borderles table-hover custom-table table-sm w-100">
                    ...
                    <tbody>
                        ...
                        <tr data-id="{{ $f->id }}" onclick="selectRow(this)">
                            ...
                            <!-- COSTO UNITARIO -->
                            <td><input name="cost_unit" type="number" class="rounded border"  placeholder="0"value="{{$f->cost_unit}}"></td>

                            <!-- COSTO TOTAL -->
                            <td><a oninput="handleInput(event)" placeholder="0">${{$f->cost_total}}</a></td>
    ...
    <div class="card border-0">
        <div class="card-header border" style="border-radius:30px " id="heading{{ $f->id }}">
            <h2 class="mb-0">
                <button class="btn btn-link" type="button" data-toggle="collapse" data-target="#collapse{{ $f->id }}" aria-expanded="true" aria-controls="collapse{{ $f->id }}">
        ...
        <div id="collapse{{ $f->id }}" class="collapse" aria-labelledby="heading{{ $f->id }}" data-parent="#accordionExample">
            <div class="card-body">
                ...
                <p><strong>Fecha entrega:</strong> <input type="date" value="{{$f->entrega}}"></p>
                ...
                <p><strong>Costo Unitario:</strong> <input type="text" class="money-input" oninput="handleInput(event)" placeholder="0"></p>
                <p><strong>Costo Total:</strong> <input type="text" class="money-input" oninput="handleInput(event)" placeholder="0" disabled></p>

                <!-- Barras de Progreso -->
                <p><strong>C. Tela:</strong>
                    <div class="progress-circle" id="progressCircle1_{{ $f->id }}">
                        <input type="number" min="0" max="100" value="0" oninput="updateProgress(this.value, 'progressCircle1_{{ $f->id }}')">
                    </div>
                </p>
                <p><strong>Costura:</strong>
                    <div class="progress-circle" id="progressCircle2_{{ $f->id }}">
                        <input type="number" min="0" max="100" value="0" oninput="updateProgress(this.value, 'progressCircle2_{{ $f->id }}')">
                    </div>
                </p>
            </div>
        </div>
